...
abstract class DataBoundObject {
   ...
   public function Load() {
       //FUNCION PARA HACER EL SELECT DE LA TABLA
      if (isset($this->ID)) {
		$strQuery = "SELECT ";
        ...
        $strQuery .= " FROM " . $this->strTableName . " WHERE user_id = :eid ORDER BY last_activity DESC LIMIT 1";
        $objStatement = $this->objPDO->prepare($strQuery);
        $objStatement->bindParam(':eid', $this->ID, PDO::PARAM_INT);
        $objStatement->execute();
        $result = $objStatement->fetchAll();
   ...
   public function sacarLista(){
    if (isset($this->ID)) {
       $strQuery = "SELECT ";
         ...
         $strQuery = substr($strQuery, 0, strlen($strQuery)-1);
         $strQuery .= " FROM " . $this->strTableName . " WHERE \"mensaje\" LIKE 'A%' order by total desc LIMIT 5";
         $objStatement = $this->objPDO->prepare($strQuery);
         //$objStatement->bindParam(':emensaje', $this->mensaje, PDO::PARAM_STR_CHAR);
         $objStatement->execute();
         $arRow = $objStatement->fetch(PDO::FETCH_ASSOC);
         foreach($arRow as $key => $value) {
             $strMember = $this->arRelationMap[$key];
